...
export default async function SignInPage(props: PageProps<"/signin">) {
  const [theme, viewer, sp] = await Promise.all([
    readThemePref(),
    getViewer(),
    props.searchParams,
  ]);
  ...
  return (
    <div className="min-h-screen bg-white text-neutral-900 dark:bg-black dark:text-white">
      <SiteHeader theme={theme} showSignup={false} viewer={viewer} />
          ...
          {isAuthed ? (
            <div>
              ...
              <p className="mt-6 max-w-md text-lg text-neutral-600 dark:text-neutral-400">
                {viewer?.inPool
                  ? "You're signed in and already in the Season 4 pool. Captains can see you — keep your ranks current before the draft."
                  : "You're signed in, but you're not in the Season 4 pool yet. Jump in so captains can scout you before the draft."}
              </p>
              ...
              <div className="mt-9 flex flex-wrap gap-3">
                ...
                <Link
                  href="/pool"
                  className="inline-flex items-center gap-2 rounded-xl border border-neutral-300 px-6 py-4 text-base font-semibold text-neutral-700 transition hover:border-thl-orange hover:text-thl-orange dark:border-neutral-700 dark:text-neutral-300"
                >
                  Browse the pool
                </Link>
              </div>
